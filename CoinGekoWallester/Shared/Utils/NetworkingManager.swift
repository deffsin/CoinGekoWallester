...
class NetworkingManager {
    ...
    private static let cache = URLCache(memoryCapacity: 500 * 1024 * 1024, diskCapacity: 1 * 1024 * 1024 * 1024, diskPath: "coinGeckoCache")
    
    static func download(url: URL, forceUpdate: Bool = false) -> AnyPublisher<Data, Error> {
        var request = URLRequest(url: url)
        request.cachePolicy = forceUpdate ? .reloadIgnoringLocalAndRemoteCacheData : .returnCacheDataElseLoad
